...
(`lib/pacing.ts`), synthetic member ids (`lib/ids.ts`), rate-limit classification
(`lib/api-football.ts`), and the federation feed's parsing and filtering
(`lib/nfsbih-feed.ts`, against a captured sample of the live feed in
